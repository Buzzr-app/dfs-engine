...
describe('dfs-payouts: lookupStandardMultiplier', () => {
  ...
  test('Underdog Standard 4-pick all-hit = 10x', () => {
    expect(
      lookupStandardMultiplier({ app: 'underdog', playType: 'underdog_standard', pickCount: 4, hits: 4 }),
    ).toBe(10);
  });
  ...
  test('lookupBaseMultiplier matches all-hit standard', () => {
    ...
    expect(lookupBaseMultiplier({ app: 'underdog', playType: 'underdog_standard', pickCount: 5 })).toBe(20);
  });
});
