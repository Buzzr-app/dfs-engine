...
describe('detectMidGameDnp — game-state gate', () => {
  test('gameState="pre" returns no-action regardless of minutes/period', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'pre', period: 1 }),
    ).toBe('no-action');
    expect(
      detectMidGameDnp({ minutesString: '0:00', gameState: 'pre', period: null }),
    ).toBe('no-action');
  });

  test('gameState="post" returns no-action (Phase C grades from gamelog after final)', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'post', period: 4 }),
    ).toBe('no-action');
    expect(
      detectMidGameDnp({ minutesString: '23:14', gameState: 'post', period: 4 }),
    ).toBe('no-action');
  });
});
...
describe('detectMidGameDnp — period gating', () => {
  test('Q1 zero minutes is no-action (early sub-rotation noise)', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'in', period: 1 }),
    ).toBe('no-action');
    expect(
      detectMidGameDnp({ minutesString: '0:00', gameState: 'in', period: 1 }),
    ).toBe('no-action');
  });

  test('Q2 zero minutes → hard-flag', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'in', period: 2 }),
    ).toBe('hard-flag');
  });

  test('Q3 zero minutes → hard-flag', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'in', period: 3 }),
    ).toBe('hard-flag');
  });

  test('Q4 zero minutes → hard-flag (zero takes precedence over soft)', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'in', period: 4 }),
    ).toBe('hard-flag');
  });

  test('OT zero minutes → hard-flag', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'in', period: 5 }),
    ).toBe('hard-flag');
  });

  test('period=null returns no-action (no quarter signal from ESPN)', () => {
    expect(
      detectMidGameDnp({ minutesString: '0', gameState: 'in', period: null }),
    ).toBe('no-action');
  });
});
...
describe('detectMidGameDnp — soft-flag boundary', () => {
  test('Q4 with 4 minutes played → soft-flag', () => {
    expect(
      detectMidGameDnp({ minutesString: '4:32', gameState: 'in', period: 4 }),
    ).toBe('soft-flag');
  });

  test('Q4 with 1 minute played → soft-flag (still under the 5-min floor)', () => {
    expect(
      detectMidGameDnp({ minutesString: '1:14', gameState: 'in', period: 4 }),
    ).toBe('soft-flag');
  });

  test('Q3 with 4 minutes played → no-action (soft only fires in Q4+)', () => {
    expect(
      detectMidGameDnp({ minutesString: '4:32', gameState: 'in', period: 3 }),
    ).toBe('no-action');
  });

  test('Q4 with exactly 5 minutes → clear (the threshold flips both flags off)', () => {
    expect(
      detectMidGameDnp({ minutesString: '5:00', gameState: 'in', period: 4 }),
    ).toBe('clear');
  });
});
...
describe('detectMidGameDnp — clear at any time', () => {
  test('Q1 with 6 minutes → clear', () => {
    expect(
      detectMidGameDnp({ minutesString: '6:14', gameState: 'in', period: 1 }),
    ).toBe('clear');
  });

  test('Q3 with 18 minutes → clear', () => {
    expect(
      detectMidGameDnp({ minutesString: '18:42', gameState: 'in', period: 3 }),
    ).toBe('clear');
  });

  test('clear fires even when period is null (we have a real MIN signal)', () => {
    expect(
      detectMidGameDnp({ minutesString: '14:00', gameState: 'in', period: null }),
    ).toBe('clear');
  });
});
...
describe('detectMidGameDnp — minutesString parser', () => {
  test('"MM:SS" truncates to coarse minutes', () => {
    ...
    expect(
      detectMidGameDnp({ minutesString: '4:59', gameState: 'in', period: 4 }),
    ).toBe('soft-flag');
    // 5:01 → 5 minutes → clear.
    expect(
      detectMidGameDnp({ minutesString: '5:01', gameState: 'in', period: 4 }),
    ).toBe('clear');
  });
  ...
  test('bare integer minutes works', () => {
    ...
    expect(
      detectMidGameDnp({ minutesString: '7', gameState: 'in', period: 2 }),
    ).toBe('clear');
  });
  ...
  test('"DNP" string is signal-less → no-action', () => {
    ...
    expect(
      detectMidGameDnp({ minutesString: 'DNP', gameState: 'in', period: 3 }),
    ).toBe('no-action');
    expect(
      detectMidGameDnp({ minutesString: 'dnp', gameState: 'in', period: 3 }),
    ).toBe('no-action');
  ...
  test('empty / dash variants are signal-less', () => {
    for (const raw of ['', ' ', '-', '—']) {
      expect(
        detectMidGameDnp({ minutesString: raw, gameState: 'in', period: 3 }),
      ).toBe('no-action');
    }
  });
  ...
  test('garbage strings fall through to no-action (defensive)', () => {
    expect(
      detectMidGameDnp({ minutesString: 'foo', gameState: 'in', period: 3 }),
    ).toBe('no-action');
    expect(
      detectMidGameDnp({ minutesString: ':30', gameState: 'in', period: 3 }),
    ).toBe('no-action');
  });
});
