...
describe('NHL skater adapters', () => {
  ...
  test('Time On Ice parses mm:ss to fractional minutes', () => {
    ...
    expect(
      extractStatForPropViaRegistry('Time On Ice', 'NHL', skater({ minutes: '20:00' }), 'prizepicks'),
    ).toBe(20);
    expect(
      extractStatForPropViaRegistry('Time On Ice', 'NHL', skater({ minutes: '0:30' }), 'prizepicks'),
    ).toBe(0.5);
  });

  test('Time On Ice returns null on DNP / "-" / malformed', () => {
    expect(extractStatForPropViaRegistry('Time On Ice', 'NHL', skater({ minutes: 'DNP' }), 'prizepicks')).toBeNull();
    expect(extractStatForPropViaRegistry('Time On Ice', 'NHL', skater({ minutes: '-' }), 'prizepicks')).toBeNull();
    expect(extractStatForPropViaRegistry('Time On Ice', 'NHL', skater({ minutes: '60:99' }), 'prizepicks')).toBeNull();
  });
...
describe('NHL Power Play Points — null-safety contract', () => {
  test('both ppg and ppa null → null (avoids silent zero-grade)', () => {
    ...
    expect(extractStatForPropViaRegistry('Power Play Points', 'NHL', noPP, 'prizepicks')).toBeNull();
  });

  test('only ppg present → ppg + 0', () => {
    const ppgOnly = skater({ fg: '2', threeP: '-' });
    expect(extractStatForPropViaRegistry('Power Play Points', 'NHL', ppgOnly, 'prizepicks')).toBe(2);
  });

  test('only ppa present → 0 + ppa', () => {
    const ppaOnly = skater({ fg: '-', threeP: '3' });
    expect(extractStatForPropViaRegistry('Power Play Points', 'NHL', ppaOnly, 'prizepicks')).toBe(3);
  });
