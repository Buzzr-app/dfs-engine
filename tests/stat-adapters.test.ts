...
function nflFixture(key: keyof typeof nflFixtures): PlayerGameLogEntryShape {
  const raw = nflFixtures[key] as unknown as PlayerGameLogEntryShape & { _player?: string; _comment?: string };
  // Strip annotation fields when constructing the runtime shape.
  const { _player: _p, _comment: _c, ...rest } = raw as PlayerGameLogEntryShape & {
    _player?: string;
    _comment?: string;
  };
  return rest;
...
describe('basketball adapters', () => {
  ...
  test('3-Pointers Made returns null on malformed threeP', () => {
    expect(
      extractStatForPropViaRegistry('3-Pointers Made', 'NBA', nbaEntry({ threeP: '-' }), 'prizepicks'),
    ).toBeNull();
  });
});
...
describe('NFL adapters — fixture-driven', () => {
  ...
  test('Pass+Rush Yds combo', () => {
    ...
    expect(extractStatForPropViaRegistry('Pass+Rush Yds', 'NFL', passOnlyQb, 'prizepicks')).toBe(380);
  });
  ...
  test('Pass+Rush+Rec Yds combo (3-way) — sums every present yardage category', () => {
    ...
    expect(extractStatForPropViaRegistry('Pass+Rush+Rec Yds', 'NFL', qb, 'prizepicks')).toBe(275 + 38);
    // Barkley: 0 pass (no pass category) + 115 rush + 28 rec = 143
    expect(extractStatForPropViaRegistry('Pass+Rush+Rec Yds', 'NFL', rb, 'prizepicks')).toBe(115 + 28);
    ...
    expect(extractStatForPropViaRegistry('Pass+Rush+Rec Yds', 'NFL', passOnlyQb, 'prizepicks')).toBe(380);
  });
  ...
  test('Pass+Rush+Rec TDs combo (3-way) — covers QB rushing/receiving outliers', () => {
    ...
    expect(extractStatForPropViaRegistry('Pass+Rush+Rec TDs', 'NFL', passOnlyQb, 'prizepicks')).toBe(4);
  });

  test('returns null when categories are absent', () => {
    expect(extractStatForPropViaRegistry('Pass Yards', 'NFL', noCategories, 'prizepicks')).toBeNull();
    expect(extractStatForPropViaRegistry('Receptions', 'NFL', noCategories, 'prizepicks')).toBeNull();
    expect(extractStatForPropViaRegistry('Pass+Rush Yds', 'NFL', noCategories, 'prizepicks')).toBeNull();
    expect(extractStatForPropViaRegistry('Pass+Rush+Rec Yds', 'NFL', noCategories, 'prizepicks')).toBeNull();
    expect(extractStatForPropViaRegistry('Pass+Rush TDs', 'NFL', noCategories, 'prizepicks')).toBeNull();
    expect(extractStatForPropViaRegistry('Pass+Rush+Rec TDs', 'NFL', noCategories, 'prizepicks')).toBeNull();
  });
...
describe('MLB batter adapters (role-discriminated)', () => {
  ...
  test('batter-only adapters return null on a pitcher row', () => {
    const pitcher = mlbPitcher({ points: '8', rebounds: '6.0', assists: '2', blocks: '1', steals: '4' });
    ...
    expect(extractStatForPropViaRegistry('Hits+Runs+RBIs', 'MLB', pitcher, 'prizepicks')).toBeNull();
  });
  ...
  test('missing stats return null', () => {
    ...
    expect(
      extractStatForPropViaRegistry('Stolen Bases', 'MLB', mlbBatter({ steals: '—' }), 'prizepicks'),
    ).toBeNull();
    expect(
...
describe('MLB pitcher adapters (role-discriminated)', () => {
  ...
  test('Pitches Thrown returns null when PC-ST was absent (mlbExtras empty)', () => {
    ...
    expect(extractStatForPropViaRegistry('Pitches Thrown', 'MLB', pitcher, 'prizepicks')).toBeNull();
    const pitcherNoExtras = mlbPitcher({ mlbExtras: undefined });
    expect(extractStatForPropViaRegistry('Pitches Thrown', 'MLB', pitcherNoExtras, 'prizepicks')).toBeNull();
  });

  test('pitcher-only adapters return null on a batter row', () => {
    const batter = mlbBatter({ points: '2', rebounds: '1', assists: '3', steals: '2', blocks: '1', turnovers: '1' });
    ...
    expect(extractStatForPropViaRegistry('Innings Pitched', 'MLB', batter, 'prizepicks')).toBeNull();
