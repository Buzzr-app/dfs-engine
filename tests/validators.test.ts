...
describe('validatePlayerGameLogEntryShape', () => {
  ...
  test('invalid mlbRole reported', () => {
    const result = validatePlayerGameLogEntryShape({ ...VALID_ENTRY, mlbRole: 'designated_hitter' });
    expect(result.ok).toBe(false);
    if (!result.ok) {
...
describe('validateDfsBetLeg', () => {
  ...
  test('bad legStatus enum value reported with allowed list', () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.errors.some((e) => e.includes('legStatus') && e.includes('pending'))).toBe(true);
    }
  });
