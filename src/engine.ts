import { computeBoostSplit, gradeDfsBetFromGraded, gradeLegFromActual, findGameLogCandidates } from './grading';
import type { BetStatus, PlayerGameLogEntryShape } from './grading';
import { lookupBaseMultiplier, lookupStandardMultiplier, recalcMultiplierAfterDnp } from './payouts';
...
export type DfsStatProviderSource = 'context.actuals' | 'context.entry' | 'entry.actualValue' | 'stat-provider';
...
export type StatProvider = {
  ...
  getGameLog?: (input: StatProviderGameLogInput) => PlayerGameLogEntryShape[] | Promise<PlayerGameLogEntryShape[]>;
};
...
export type DfsEngine = {
  ...
  gradeLeg(leg: DfsLegInput, actual: number | null, provenance?: DfsProvenance | null): DfsLegDecision;
...
export function defineLeagueAdapter(adapter: DfsLeagueAdapterDefinition): DfsLeagueAdapterDefinition {
...
const DEFAULT_BOOK_POLICIES: DfsBookPolicy[] = [
  ...
  },
  {
    ...
    version: '2026-05',
    effectiveFrom: '2026-05-01',
    sourceNotes: ['Default v2 policy profile; provider-specific rescued-pick inputs are modeled as DNP removals.'],
    dnp: { removeLeg: true, voidIfNoSurvivors: true },
    push: { removeLeg: true, refundIfNoSurvivors: true },
  },
];
...
export function createDfsEngine(config: DfsEngineConfig = {}): DfsEngine {
  ...
  for (const policy of config.bookPolicies ?? []) policies.set(policy.app, defineBookPolicy(policy));
  for (const provider of config.statProviders ?? []) statProviders.set(provider.id, defineStatProvider(provider));
  ...
  const lookupConfiguredMultiplier = (opts: {
    ...
    for (let i = payoutTables.length - 1; i >= 0; i -= 1) {
      ...
      const entry = table.entries.find((row) => row.pickCount === opts.pickCount && row.hits === opts.hits);
  ...
  const normalizeEntry = (input: DfsEntryInput): DfsEntryInput => {
    const baseMultiplier =
      ...
      lookupBaseMultiplier({
        ...
        pickCount: input.legs.filter((candidate) => candidate.legStatus !== 'dnp').length || input.legs.length,
      });
    return {
      ...input,
      baseMultiplier,
  ...
  const extractLegStat = async (
    ...
    const key = asDfsPropTypeKey(leg.propType);
    const hasContextActual = Object.prototype.hasOwnProperty.call(context.actualsByLegId ?? {}, leg.legId);
  ...
  const settleEntry = async (
    ...
    audit(events, 'entry.normalized', normalized.entryId ?? null, `${normalized.legs.length} legs normalized`, context);
    ...
    for (const currentLeg of normalized.legs) {
      if (currentLeg.legStatus === 'dnp') {
        ...
        audit(events, 'entry.adjusted', normalized.entryId ?? null, 'leg removed as DNP', context, currentLeg.legId);
        continue;
      }
      ...
      if (!stat.ok) {
        ...
        audit(events, 'leg.actual.pending', normalized.entryId ?? null, stat.detail, context, currentLeg.legId);
        continue;
      }
      ...
      audit(events, 'leg.graded', normalized.entryId ?? null, decision.status, context, currentLeg.legId);
    }
    ...
    const surviving = decisions.filter((decision) => decision.status !== 'dnp' && decision.status !== 'push');
    if (surviving.length === 0) {
      const status: BetStatus = decisions.some((decision) => decision.status === 'dnp') ? 'void' : 'pushed';
    ...
    const graded = gradeDfsBetFromGraded({
      ...
      displayedMultiplier,
      baseMultiplier: normalized.baseMultiplier ?? lookupConfiguredMultiplier({
        app: normalized.app,
        playType: normalized.playType,
